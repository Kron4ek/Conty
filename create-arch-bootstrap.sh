...
mkdir -p mountpoint
mount "${bootstrap}"/usr/share ./mountpoint && cd ./mountpoint || exit 1
cd ./doc && for d in *; do if [ "$d" != "*bottles*" ]; then rm -Rf "$d"; fi done && cd .. || exit 1
rm -Rf "$(find ./locale/*/*/* | grep -v "bottles" | sort | tr '\n' ' ')"
cd .. && umount /mountpoint || exit 1
